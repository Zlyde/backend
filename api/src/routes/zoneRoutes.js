...
router.put("/:id", async (req, res) => {
  try {
    const updatedZone = await parkingZoneService.updateParkingZone(
      req.params.id,
      req.body,
    );
...
router.delete("/:id", async (req, res) => {
  try {
    const deletedZone = await parkingZoneService.deleteParkingZone(
      req.params.id,
    );
    if (!deletedZone)
...
router.get("/:id/bikes", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Error fetching bikes for parking zone with ID ${zoneId}:`,
      error.message,
    );
