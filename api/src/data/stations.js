...
const getChargingStationById = async (id) => {
  try {
    ...
    console.error(
      `Error fetching charging station by ID ${id}:`,
      error.message,
    );
    throw new Error(error.message);
  }
...
const updateChargingStation = async (id, stationDataToUpdate) => {
  try {
    ...
    console.error(
      `Error updating charging station with ID ${id}:`,
      error.message,
    );
    throw new Error(error.message);
  }
...
const deleteChargingStation = async (id) => {
  try {
    const deletedStation = await ChargingStation.findOneAndDelete({
      charging_station_id: id,
    });
    if (!deletedStation) {
      throw new Error(`Charging station with ID ${id} not found`);
    ...
    console.error(
      `Error deleting charging station with ID ${id}:`,
      error.message,
    );
    throw new Error(error.message);
  }
...
const getBikesAtChargingStation = async (stationId) => {
  try {
    ...
    console.error(
      `Error fetching bikes at charging station with ID ${stationId}:`,
      error.message,
    );
    throw new Error(error.message);
  }
