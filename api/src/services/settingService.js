...
const updateSettings = async (settingsToUpdate) => {
    try {
        // Validera base_price
        if (settingsToUpdate.base_price !== undefined && settingsToUpdate.base_price < 0) {
            throw new Error('Base price cannot be negative.');
        }

        // Validera price_per_minute
        if (settingsToUpdate.price_per_minute !== undefined && settingsToUpdate.price_per_minute < 0) {
            throw new Error('Price per minute cannot be negative.');
        }

        // Validera rules, om de anges
        if (settingsToUpdate.rules) {
            const { free_parking_duration, fine_for_wrong_parking } = settingsToUpdate.rules;
