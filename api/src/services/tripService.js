...
const endTrip = async (tripId) => {
  ...
  if (bike.battery_level < 50) {
    ...
    const isInChargingStation = await geoService.isInChargingStation(
      updatedTripData.end_location.coordinates,
    );
    ...
    if (newStatus === "maintenance") {
      console.log(
        `Bike ${bike.bike_id} set to maintenance due to low battery.`,
      );
    }
  } else {
  ...
  if (user.role === "customer") {
    ...
    console.log(
      `For admin user ${user.user_id} this trip is free. No invoice created.`,
    );
  }
