...
const createInvoice = async (tripId, userId) => {
  try {
    ...
    if (!tripId || !userId) {
      throw new Error(
        "Both tripId and userId are required to create an invoice.",
      );
    }
    ...
    const isInParkingZone = await geoService.isInParkingZone(
      trip.start_location.coordinates,
    );
    console.log(isInParkingZone);

    const isInChargingStation = await geoService.isInChargingStation(
      trip.start_location.coordinates,
    );
    ...
    let fee = 0;
    const isEndInParkingZone = await geoService.isInParkingZone(
      trip.end_location.coordinates,
    );
    const isEndInChargingStation = await geoService.isInChargingStation(
      trip.end_location.coordinates,
    );
...
const markInvoiceAsPaid = async (invoiceId) => {
  try {
    ...
    if (invoice.status === "paid") {
      throw new Error(
        `Invoice with ID ${invoiceId} is already marked as paid.`,
      );
    }
    ...
    if (!paymentMethod) {
      throw new Error(
        `User with ID ${user.user_id} has not selected a payment method.`,
      );
    }
    ...
    if (paymentMethod === "prepaid") {
      ...
      throw new Error(
        `Invalid payment method for user ${user.user_id}: ${paymentMethod}`,
      );
    }
    ...
    console.log(
      `Invoice ${invoiceId} marked as paid with payment method: ${paymentMethod}`,
    );
