...
describe("geoService", () => {
  ...
  describe("getBikesInCity", () => {
    ...
    it("should throw an error if cityId is invalid", async () => {
      ...
      await expect(geoService.getBikesInCity(cityId)).rejects.toThrow(
        "City ID is invalid",
      );
    });
    ...
    it("should throw an error if no bikes are found", async () => {
      ...
      await expect(geoService.getBikesInCity(cityId)).rejects.toThrow(
        "No bikes found in city with ID 123",
      );
    });
  });
  ...
  describe("getBikesInChargingStation", () => {
    ...
    it("should throw an error if stationId is invalid", async () => {
      ...
      await expect(
        geoService.getBikesInChargingStation(stationId),
      ).rejects.toThrow("Charging station ID is invalid");
    });
    ...
    it("should throw an error if no bikes are found", async () => {
      ...
      await expect(
        geoService.getBikesInChargingStation(stationId),
      ).rejects.toThrow("No bikes found in charging station with ID 456");
    });
  });
  ...
  describe("getBikesInParkingZone", () => {
    ...
    it("should throw an error if zoneId is invalid", async () => {
      ...
      const zoneId = null;
      await expect(geoService.getBikesInParkingZone(zoneId)).rejects.toThrow(
        "Parking zone ID is invalid",
      );
    });
    ...
    it("should throw an error if no bikes are found", async () => {
      ...
      // Förvänta oss att funktionen kastar ett fel
      await expect(geoService.getBikesInParkingZone(zoneId)).rejects.toThrow(
        "No bikes found in parking zone with ID 789",
      );
    });
  });
});
